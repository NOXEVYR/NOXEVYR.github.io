...
document.querySelector('#update-list').innerHTML=recent.map(p=>`<a class="update-row" href="https://github.com/turnsolesama/${p.id}/releases" ${external}><time class="update-date" datetime="${esc(p.date)}">${esc(p.date.replaceAll('-','.'))}</time><div><h3>${esc(p.name)} <span>v${esc(p.version)}</span></h3><p>${esc(p.update)}</p></div><span class="update-arrow" aria-hidden="true">↗</span></a>`).join('');
