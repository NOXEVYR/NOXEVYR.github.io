...
function projectIcon(p,loading='lazy'){return `<span class="project-icon software-icon icon-${esc(p.id)}"><img src="${esc(p.icon)}" alt="${esc(p.name)}图标" width="44" height="44" loading="${loading}" decoding="async"></span>`}
function projectImages(p){
 const images=p.screenshots?.length?p.screenshots:p.image?[{src:p.image,caption:p.imageNote}]:[];
 return images.map((image,index)=>`<figure class="project-preview"><a href="${esc(image.src)}" ${external} aria-label="查看${esc(p.name)}：${esc(image.caption)}原图"><img class="dialog-image" src="${esc(image.src)}" alt="${esc(p.name)} · ${esc(image.caption)}" ${image.width&&image.height?`width="${esc(image.width)}" height="${esc(image.height)}"`:''} loading="${index?'lazy':'eager'}" decoding="async"></a><figcaption class="image-note">${esc(image.caption)} <span>点击图片查看原图 ↗</span></figcaption></figure>`).join('');
}
...
// Repository-hosted packages may have an obsolete migration entry on the Releases page.
function updateLink(p){const url=p.downloads[0]?.url;return url?.startsWith('https://github.com/')&&url.includes('/releases/tag/')?url:`https://github.com/NOXEVYR/${p.id}#readme`;}
document.querySelector('#update-list').innerHTML=recent.map(p=>`<a class="update-row" href="${esc(updateLink(p))}" ${external}><time class="update-date" datetime="${esc(p.date)}">${esc(p.date.replaceAll('-','.'))}</time><div><h3>${esc(p.name)} <span>v${esc(p.version)}</span></h3><p>${esc(p.update)}</p></div><span class="update-arrow" aria-hidden="true">↗</span></a>`).join('');
